...
def breadthFirstSearch(problem):
    ...
    queue = util.Queue()
    start_state = problem.getStartState()
    queue.push((start_state, []))  # (state, path)
    visited = set()
...
def uniformCostSearch(problem):
    ...
    start_state = problem.getStartState()
    priority_queue.push((start_state, []), 0)  # (state, path), cost
    visited = set()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    priority_queue = util.PriorityQueue()
    start_state = problem.getStartState()
    priority_queue.push((start_state, []), heuristic(start_state, problem))  # (state, path), priority
    visited = set()
    cost_so_far = {start_state: 0}
